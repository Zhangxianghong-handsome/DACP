...
transformer = torchvision.transforms.Compose(
    [torchvision.transforms.ToTensor(), torchvision.transforms.Normalize(0.1307, 0.3018)])

train_data = datasets.MNIST('MNIST', True, transformer, download=True)
test_data = datasets.MNIST('MNIST', False, transformer, download=True)
...
if __name__ == '__main__':
    ...
    for i in range(EPOCHS):
        ...
        train()
        acc.append(test())
    with open('OUTPUT_CODESIGN/mnist.csv', mode='w', newline='') as file:
    ...
    torch.save(mnistnet.state_dict(), 'OUTPUT_CODESIGN/mnistnet_weights.pth')
